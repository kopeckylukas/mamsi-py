...
class MamsiStructSearch:
    ...
    def _get_adduct_groups(self):
        ...
        for index, frame in enumerate(self.assay_metadata):
            ...
            for item in non_unique_features:
                ...
                fr_ = working_frame[working_frame['Feature'] == item].loc[:, ['Feature', 'Adduct cluster', 'Adduct']]
                ...
                for i in range(len(fr_) - 1):
                    working_frame['Adduct cluster'].replace({fr_.iloc[i + 1, 1]: fr_.iloc[0, 1]}, inplace=True)
                    # working_frame.reset_index(inplace=True, drop=True)
    ...
    def get_structural_clusters_original(self, detect_iso=True, adducts='all', unify_overlap=True, annotate=True,
        ...
        for frame in data:
            ...
            if return_as_single_frame:
                ...
                working_frame['Adduct cluster'] = working_frame['Adduct cluster'].apply(lambda x: x + adduct_offset)
                ...
                adduct_offset = working_frame['Adduct cluster'].max()  # Update offset
    ...
    def get_neutral_mass(self, features, adducts='all'):
        ...
        :param features: dataframe with m/z
    ...
    def _get_unified_struct_groups(self):
        """
        Functions unifies 'Isotopologue group' and 'Adduct cluster' clusters into a single cluster
        :param frame: frame with Isotopologue groups and adduct clusters
        :return: frame with unified structural clusters
        """
        for index, frame in enumerate(self.assay_metadata):
            ...
            frame_['Adduct cluster'] = frame_['Adduct cluster'].apply(lambda x: x+offset)  # adduct clusters above iso
            adduct_frame = frame_.dropna(subset=['Adduct cluster'])  # Get only rows with non 0 clusters
            iso_frame = frame_.dropna(subset=['Isotopologue group'])  # Get only rows with non 0 clusters
            adduct_frame.insert(0, 'Structural cluster', adduct_frame['Adduct cluster'])  # Create structural cluster column
